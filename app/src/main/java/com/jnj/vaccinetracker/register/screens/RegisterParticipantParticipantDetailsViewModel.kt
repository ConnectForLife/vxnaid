...
class RegisterParticipantParticipantDetailsViewModel @Inject constructor(
    ...
    data class Args(
            ...
            val isDuplicateIdEdgeCase: Boolean = false,
    )
    ...
    val isDuplicateIdEdgeCase = mutableLiveData(false)
    ...
    private suspend fun load(args: Args) {
        ...
        try {
            ...
            isDuplicateIdEdgeCase.set(args.isDuplicateIdEdgeCase)
    ...
    private suspend fun doRegistration(
        ...
        try {
            ...
            val result: DraftParticipant = if (childUuid != null) {
                if (isDuplicateIdEdgeCase.value == true) {
                    val draft: DraftParticipant? = draftParticipantRepository.findByParticipantUuid(childUuid)

                    draft?.let {
                        val draftToUpdate = it.copy(participantId = participantId)
                        draftParticipantRepository.updateDraftParticipant(draftToUpdate)
                    }
                    ...
                    val keysToDelete = errorKeys.filter { key ->
                        key.contains(draft?.participantUuid.toString())
                    }
